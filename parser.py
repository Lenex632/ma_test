...
def get_data(store_id: int, slug: str) -> dict:
    ...
    query = {
        ...
        "variables": {
            ...
            "slug": slug,
            "from": 0,
            "size": 10000,  # не совсем уверен, что с этим можно сделать (в API этот параметр обязательный, однако значения как "выбрать всё" нет). Возможно следует записать максимально возможный вариант из документации (2^31 - 1), но я решил, что для тестового задания должно хватить 10000
